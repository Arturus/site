...
месту жительства.
Чтобы рекламная компания была эффективной, она часто должна быть таргетирована
...
Посты в Instagram могут содержать информацию о месте, в котором был сделан пост, называемую *geotags*.
 Это самый простой и очевидный способ определения geolocation. В заголовке статьи приведена карта,
...
* Часто люди используют geotagging, только когда едут в командировку/отпуск/путешествие или находятся в интересных с их точки зрения местах.
 То есть у блоггера, живущего в Лондоне, геотэги могут быть из каких угодно мест мира, кроме самого Лондона.
...
Cуществует много дополнительных, не столь очевидных и надёжных, как геотэги, источников информации о местоположении блоггера.

* В первую очередь это текст из раздела ‘bio’ аккаунта: в нем могут быть указаны город блоггера или локальные географические названия, 
...
что например японская аудитория скорее будет у блоггера из Японии, чем у блоггера из Мексики.
Также маловероятно, что мексиканский блоггер будет делать посты на японском языке.
...
но совместное их использование может дать намного более более точный результат. Классический подход, 
...
Простейший способ создания meta classifier - это использование *weighted majority voting*.
 ...
 вероятности правильной классификации $\hat{p}_i$ (в данном случае правильному определению страны блоггера).
 Вероятности рассчитываются по обучающей выборке.
$$w_i=\log\left(\frac{\hat{p}_i}{1−\hat{p}_i}\right),\quad i=1,\dotsc,N$$

Классификаторы, которые часто ошибаются, получают меньший вес, классификаторы, у которых высокая точность &mdash; больший вес. 
...
* Предсказания base classifiers могут быть неверными, например из bio может быть извлечена страна Япония, 
 из геотэгов - Бразилия и Германия, при этом блоггер на самом деле живет в Англии.
...
Проблема в том, что распределение стран по кол-ву аккаунтов в них
...
На рис. 1 видно, что с уменьшением кол-ва аккаунтов на страну точность понижается. 
...
Как видно из рис. 3, наиболее важны country_id (идентификатор страны, позволяет модели
 учитывать индивидуальные особенности отдельных стран), геотэги из постов, язык блоггера
...
блоггеров и их аудиторий, и убрав больше количество претензий клиентов к неточным данным.
