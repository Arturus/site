...
анализировать единственное измерение недостаточно. 
...
Теоретически можно построить отчёт одновременно по двум измерениям,
 ...
 от чего на самом деле зависит конверсия, для обычного человека нереально: слишком много данных. 

Нужны более совершенные автоматические способы оценки воздействия факторов,
...
Сумма всех факторов каждого $j$-го посетителя, умноженных на их веса, даст
значение $z_j$, называемое также *logits*. Факторы в нашем случае задаются
индикаторными переменными, принимающими значения 1 (фактор присутствует) и 0 (фактор отсутствует). Для приведённого 
выше примера было бы четыре фактора `город=Москва`, `город=Екатеринбург`,
                                    `источник=Organic`, `источник=Referral` 
...
$$z\_j = w\_1x\_{j,1} + w\_2x\_{j,2} + w\_3x\_{j,3} + w\_4x\_{j,4} + b$$
или в обобщённой векторной записи:
$$z_j=\mathbf{w}^\top\mathbf{x_j} + b$$ 

Переменные, формирующие вектор $\mathbf{x_j}$, характеризующий каждого посетителя сайта,
в машинном обучении называются *признаками*. 
...
Настоящих адресов раскрывать не будем, т.к. в результатах работы моделей
...
Чтобы понять, насколько модель соотносится с реальной жизнью, будем
...
мы будем использовать в основном его. Прогноз из "прошлого" в "будущее" является
...
значения, встречающиеся реже определённого порога, помещены в отдельную категорию `(other)`, 
...
с помощью [one-hot кодирования](https://en.wikipedia.org/wiki/One-hot).
...
1. Линейная модель хорошо работает только для самых простых прогнозов.
...
2. Если имеются нелинейные зависимости между фактором и целевой переменной, или факторов друг c другом,
...
2. Использовать сложную модель, которая может обучиться потенциально
...
Но существует альтернатива, позволяющая совместить и сложность
...
от конкретного посетителя, например вклад от признака `city:Могилёв`
...
Но недавно были разработаны способы эффективного расчёта векторов Шэпли
...
Таким образом мы получаем ещё и приблизительную оценку уверенности в степени воздействия фактора. 
...
логистической регрессии, которую мы применяли, работая с линейной моделью:
 ...
 изменение шансов в $n$ раз эквивалентным изменению конверсионности в $n$ раз ($1.98\% \approx 2 \times 1\%$), тогда:
...
Такие продвинутые модели мы рассмотрим в следующих статьях ([1](/post/conversion_history),
[2](/post/clv), [3](/post/user_churn)).
